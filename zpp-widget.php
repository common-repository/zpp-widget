...
class Zpp_Widget extends WP_Widget {
	...
	function save_cover_to_cache( $url ) {
		...
		if(is_writable(ABSPATH . "/wp-content/zpp-cache/covers")) {
			...
			if( ( !fwrite( $fp, $data) ) && ( get_option( 'zpp_debug' ) == 'true' ) ) {
	...
	public static function save_xml_to_cache( $url, $xml ) {
		...
		if(is_writable(ABSPATH . "/wp-content/zpp-cache/xml")) {
			...
			if( ( !fwrite( $fp, $xml) ) && ( get_option( 'zpp_debug' ) == 'true' ) ) {
...
<?php
	...
	if( isset( $_POST[$hidden_debug_name] ) && $_POST[ $hidden_debug_name ] == 'Y' ) {
			update_option( $debug_opt_name, $_POST['debug'] );
			$debug_opt_val = $_POST['debug'];
			$data_saved = true;
	}
...
<p>Jeśli wtyczka nie działa poprawnie, to włącz poniższą opcję i wyślij komunikat błędu ze strony na adres pp@zlotemysli.pl</p>
<form name="formdebug" method="post" action="">
<input type="hidden" name="<?php echo $hidden_debug_name; ?>" value="Y" />
<label for="debug">TRYB AWARYJNY</label>
<input type="checkbox" name="debug" value="true" <?php if($debug_opt_val == 'true') echo 'checked';?> />
<p class="submit">
<input type="submit" name="Submit" class="button-primary" value="Zapisz" />
</p>
</form>
